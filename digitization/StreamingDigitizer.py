...
class Digitizer:
    ...
    def digitize(self, path: str, return_values=False, force_second_contour=False):
        """
        ...
        frame = ecg.copy()
        try:
            if force_second_contour:
                raise ValueError("This is a forced contour error.")
            # Preprocess
            ...
            ecg_crop, rect = self.__preprocessor.preprocess_hierarchy(ecg)
            # Extraction
            raw_signals = self.__signal_extractor.extract_signals(
                ecg_crop
            )
