...
class Preprocessor:
    ...
    def preprocess_hierarchy(self, ecg: Image) -> Tuple[Image, Rectangle]:
        """
        Preprocess and ECG image.

        Used as backup in case external contour returns entire ECG.
        This sometimes happens when the metadata is encased in a box
        ...
        """
        ecg = ecg.copy()
        rect = self.__img_partitioning_hierarchy(ecg)
        ecg.crop(rect)
        ecg = self.__gridline_removal(ecg)
        return (ecg, rect)
    
    def __img_partitioning_hierarchy(self, ecg: Image) -> Rectangle:
        """
        Get the rectangle which contains the grid with the ECG signals.
        ...
        # Change the contour retrieval mode to cv.RETR_CCOMP
        contours, hierarchy = cv.findContours(
            edges.copy(), cv.RETR_CCOMP, cv.CHAIN_APPROX_NONE
        )
        
        # Collect all bounding rectangles (regardless of hierarchy level)
        all_rects = [cv.boundingRect(contour) for contour in contours]
        
        # Sort rectangles by area (width * height) and take the top 3
        sorted_rects = sorted(all_rects, key=lambda x: x[2] * x[3], reverse=True)
        second_largest_rect = sorted_rects[1]
        x, y, w, h = second_largest_rect
        rect = Rectangle(Point(x, y), Point(x + w, y + h))
        return rect
